 ...
 <form id="xjrw" action="" method="post">
  <table width="90%" height="70%" align="center">
  	<input type="hidden" name="id" value="${schedule.id}">
        <tr>
        	<td colspan="2">任务名称:<input type="text" class="input1" id="name" name="name" value="${schedule.name}"></td>
        </tr>
        <tr>
        	<td colspan="2">开始时间:<input style="width: 170px;" readonly="readonly" type="text" id="expectEndTime" name="startDate" 
	                         onclick="WdatePicker()" onfocus="WdatePicker({skin:'whyGreen',dateFmt:'yyyy-MM-dd HH:mm:ss'})" class="Wdate" value="${schedule.startDate}">
			</td>
        </tr>
        <tr>
        	<td colspan="2">结束时间:<input style="width: 170px;" readonly="readonly" type="text" id="expectEndTime" name="endDate" 
	                         onclick="WdatePicker()" onfocus="WdatePicker({skin:'whyGreen',dateFmt:'yyyy-MM-dd HH:mm:ss'})" class="Wdate" value="${schedule.endDate}">
			</td>
        </tr>
        <tr>
        	<td>是否提醒： <input type="checkbox" value="1" <c:if test="${schedule.remind =='1'}"> checked="checked" </c:if> name="remind">
        	</td>
        	<td>优先级：
        				<select name="priority" class="input-medium">
        	  				<option value='1' <c:if test="${schedule.priority =='1'}"> selected="selected" </c:if>>高</option>
        	  				<option value='2' <c:if test="${schedule.priority =='2'}"> selected="selected" </c:if>>中</option>
        	  				<option value='3' <c:if test="${schedule.priority =='3'}"> selected="selected" </c:if>>低</option>
        			   </select>
            </td>
        </tr>
        <tr>
        	<td colspan="2">
        		备注：<textarea class="textarea_refuse" name="remarks">${schedule.remarks}</textarea>
        	</td>
        </tr>
        <tr>
