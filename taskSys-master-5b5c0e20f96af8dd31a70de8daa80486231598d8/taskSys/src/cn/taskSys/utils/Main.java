...
public class Main {
	...
	public List<ChinaDate> getCurrentDateInfo() {
		...
		try {
			...
			// Thread.sleep(8000);
			DomNodeList<HtmlElement> htmlElements = page.getElementById(
					"M-dates").getElementsByTagName("li");
			// System.out.println(htmlElements.size());
			for (HtmlElement element : htmlElements) {
				ChinaDate chinaDate = new ChinaDate();
				List<HtmlElement> lunar = element.getElementsByAttribute(
						"span", "class", "lunar");
				...
				chinaDate.setSolarDate(dateFormat.parse(element
						.getAttribute("date")));
				if (element.getAttribute("class").indexOf("vacation") != -1) {
					chinaDate.setVacation(true);
					chinaDate.setVacationName(this.getVocationName(
	...
	public ChinaDate getTodayInfo() {
		...
		for (ChinaDate date : dateList) {
			if (dateFormat.format(date.getSolarDate()).equals(
