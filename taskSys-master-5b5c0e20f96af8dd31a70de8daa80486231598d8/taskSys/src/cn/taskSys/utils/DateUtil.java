...
public class DateUtil {
	...
	//获得本月的第一天
		public static Date getFdayInMonth(Date nowtime) throws ParseException{
			Calendar a=Calendar.getInstance(); 
			a.set(Calendar.DATE, 1);//把日期设置为当月第一天 
			Date thedate=a.getTime();
			return thedate;
		}
		//获得本月的最后一天
		public static Date getLdayInMonth(Date nowtime) throws ParseException{
			Calendar a=Calendar.getInstance(); 
			int month=a.get(Calendar.MONTH);		
			a.set(Calendar.MONTH,month+1);//把日期设置为当月第一天
			a.set(Calendar.DATE, 1);//把日期设置为当月第一天
	...
	/**
	 * 格式不规则日期：yyyy-MM-dd HH:mm:ss
	 * @param date
	 * @return
	 */
	public static String retStrTime(Date date){
		String retTime = "";
		try {
	...
	/**
	 * 
	 * @param i 1下一月  0当前月    -1上一月
	 * @param pattern 返回日期格式，如：yyyy-MM-dd HH:hh:ss
	 * @return 日期字符串
	 */
	public static String getDate(int i, String pattern){
		SimpleDateFormat monthFormat = new SimpleDateFormat(pattern);
		Calendar calendar = Calendar.getInstance();  
		Date nowDate = new Date();
		calendar.setTime(nowDate);  
		calendar.add(Calendar.MONTH, i); //1下一月  0当前月    -1上一月
	 ...
	 * 
	 */
	public static int getAllDays(String strStartDate,String strEndDate) {  
		SimpleDateFormat df = new SimpleDateFormat("yyyy-MM-dd");  
		Date startDate=null;  
		Date endDate = null;  
		...
		try {  
			...
			if(startDate.compareTo(endDate)<=0){
				...
				result = new Long(days).intValue()+1;
			}else{
				System.out.println("开始日期必需小于等于结束日期！");
			}
	 ...
	 * @return 日期 
	 */
	public static Date getNowTime(int i){
	...
	public static void main(String[] args) {
		System.out.println(getAllDays("2016-09-02", "2016-09-05"));
	}
