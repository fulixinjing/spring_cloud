...
<form id ="f1" action="${ctx}/taskListQuery/listTask.do" method="POST">
...
<div class="TAB_right">
	<div class="retrieve_list1">
			<table class="retrieve_table" width="100%" cellpadding="0" cellspacing="0">
					<tr>					
						<td>
							创建人：
							<input type="text" id="create_name" name="create_name"  style="width:70px" value="${create_name}" onblur="validateXml1(this)"> </td>
						<td> 所属部门：
								<select name="department" id="department" class="sel" style="width:170px" onchange="toChnage(this.value)">
                              <c:choose>
					    ...
					    <td>
					     ...
					     所属团队：<select name="team" id="team" style="width: 100px;" class="sel">
                               <c:choose>
                              	<c:when test="${userRole=='staff' || userRole=='TDJL' }">
                              		<c:forEach items="${dictionaryLlist}" var="dic" varStatus="status">
					...
					<tr>
					    <td>
					 		负责人：
					 			<input class="srk" type="text" style="width: 70px;" id="exec_name" name="exec_name" value="${exec_name }" onblur="validateXml1(this)">   
					   </td>
						
						<td> 创建时间：<input style="width: 75px;" readonly="readonly"  type="text" id="startTime" name="startTime" value= "${taskInfo.startTime}"  onclick="WdatePicker({dateFmt:'yyyy-MM-dd',isShowClear:true});"/>
						至&nbsp;&nbsp;<input style="width: 75px;" readonly="readonly"  type="text" id="endTime" name="endTime" value="${fn:substring(taskInfo.endTime, 0, 10)}"  onclick="WdatePicker({dateFmt:'yyyy-MM-dd',isShowClear:true});"/>						
						</td>		
						
						<td>
					 		任务内容：<input class="srk" type="text" style="width: 100px;" id="taskContent" name="taskContent" value="${taskInfo.taskContent }" onblur="validateXml1(this)" onkeydown="disableBackspace();" onkeyup="disableBackspace();">   
					   </td>	
					   <td>
					 		项目名称：<input class="srk" type="text" style="width: 90px;" id="projectName" name="projectName" value="${taskInfo.projectName }">
        		...
        		<table class="fancyTable" id="myTable02" cellpadding="0" cellspacing="0" style="table-layout:fixed;">
        		    ...
        		    <c:forEach items="${pageView.records}" var="task" varStatus="status">
        		        <tr>
                            ...
                            <td >${task.score }</td>			
                            <td nowrap="nowrap">${task.taskworktime }</td>
...
<script type="text/javascript">
	...
	function selectSubmit(){
       ...
       var lowScore = $("#lowScore").val();
		...
		$("#nodeId").val('');
		$("#f1").attr("method","POST");
		$("#f1").attr("action","${ctx}/taskListQuery/listTask.do");
		$("#f1").submit();
		layer.load('加载中...',0);
	}	
	...
	function topage(page){
		...
		$("#listForm").attr("action","${ctx}/taskListQuery/listTask.do");
		$("#listForm").submit();
	}	
	//到指定的分页页面
	...
	function delTasks(){
		...
		if(isDelete){
			$("#f1").attr("method","POST");
			$("#f1").attr("action","${ctx}/admin/delTasks.do?flagfrom=1&ids="+ids);
	    	$("#f1").submit();
		}
	}
	//排序
